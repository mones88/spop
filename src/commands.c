...
#define jb_add_double(jb, name, val) {\
    json_builder_set_member_name(jb, name); \
    json_builder_add_double_value(jb, val); }
...
typedef enum {
    UICRT_ERROR = -1, UICRT_DONE, UICRT_WAIT
} uri_image_cb_result_type;
typedef struct {
    command_context *ctx;
    sp_link *link;
    sp_image_size size;
...
static void json_playlist_offline_status(sp_playlist *pl, JsonBuilder *jb) {
    ...
    switch (pos) {
        case SP_PLAYLIST_OFFLINE_STATUS_NO:
            json_builder_add_boolean_value(jb, FALSE);
            break;
        ...
        case SP_PLAYLIST_OFFLINE_STATUS_WAITING:
            json_builder_add_string_value(jb, "waiting");
            break;
        default:
...
gboolean command_run(command_finalize_func finalize, gpointer finalize_data, command_descriptor *desc, int argc,
                     char **argv) {
    gboolean ret = TRUE;
...
gboolean list_playlists(command_context *ctx) {
    ...
    TidalServiceResult *serviceResult = tidal_session_get_user_playlists();

    if (serviceResult->firstItem != NULL) {
        TidalListItem *playlistItem = serviceResult->firstItem;
        while (playlistItem != NULL) {
            TidalPlaylist *playlist = playlistItem->item;
            json_builder_begin_object(ctx->jb);
            jb_add_string(ctx->jb, "type", "playlist");
            jb_add_string(ctx->jb, "name", playlist->name);
            jb_add_int(ctx->jb, "tracks", playlist->num_tracks);
            jb_add_bool(ctx->jb, "offline", FALSE);
...
gboolean status(command_context *ctx) {
    sp_track *track;
    int track_nb, total_tracks, track_popularity;
    guint track_duration;
    guint64 track_position;
    queue_status qs;
    gchar *track_name;
    gchar *track_artist;
    gchar *track_album;
...
gboolean uri_info(command_context *ctx, sp_link *lnk) {
    ...
    switch (type) {
        case SP_LINKTYPE_INVALID: jb_add_string(ctx->jb, "type", "invalid");
            sp_link_release(lnk);
            break;
        ...
        default: jb_add_string(ctx->jb, "type", "not implemented");
            sp_link_release(lnk);
            break;
    }
...
static gboolean _uri_add_play(command_context *ctx, sp_link *lnk, gboolean play) {
    ...
    switch (type) {
        case SP_LINKTYPE_INVALID: jb_add_string(ctx->jb, "error", "invalid URI");
            sp_link_release(lnk);
            break;
        case SP_LINKTYPE_TRACK: {
            int offset;
        ...
        default: jb_add_string(ctx->jb, "error", "not implemented");
            sp_link_release(lnk);
            break;
    }
...
gboolean uri_image_size(command_context *ctx, sp_link *lnk, guint size) {
    ...
    switch (type) {
        ...
        default: jb_add_string(ctx->jb, "error", "link not supported");
            sp_link_release(lnk);
            break;
    }
